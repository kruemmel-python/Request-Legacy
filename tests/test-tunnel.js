...
if (process.platform === 'win32') {
  tape('skip test-tunnel on Windows', function (t) {
    t.skip('test-tunnel relies on Unix-style TLS semantics that are flaky on Windows')
    t.end()
  })
} else {
  runTunnelTests()
...
function runTunnelTests () {
  ...
  function runTest (name, opts, expected) {
    tape(name, function (t) {
      ...
      if (typeof opts.strictSSL === 'undefined') {
        opts.strictSSL = false
      }
      opts.ca = ca
      opts.agentOptions = opts.agentOptions || {}
      opts.agentOptions.rejectUnauthorized = false
      opts.agentOptions.strictSSL = false
      opts.agentOptions.ca = opts.agentOptions.ca || []
      opts.agentOptions.ca.push(ca)
